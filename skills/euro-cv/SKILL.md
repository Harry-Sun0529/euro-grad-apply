...
description: >
  ...
  当用户输入 /euro-cv，或说"帮我做简历"、"生成 CV"、"针对 XX 项目改简历"、"CV 排版"时触发。
  make me a CV、tailor my CV for a program。
---
...
如果用户只是问 CV 写法、照片惯例或如何导出 PDF，而不是要求生成成品，应转交 `/euro-docs` 的 CV 方法路径；无文件读写能力时也只提供方法，不承诺文件生成。
