...
> 📌 **如果你的 AI 限制知识库文件数量**（如 Gemini Gem 只允许 10 个文件）：
> 不用传全部 27 个文件——直接下载仓库里的单文件 **[`euro-grad-apply-full.md`](euro-grad-apply-full.md)**（已把所有内容合并成一个文件），只传这一个即可。
...
- 各大学及机构官网（英文/当地语言）
- 官方政策文件与新闻报道
- 中文留学社区公开讨论（小红书、知乎、一亩三分地、豆瓣、Reddit等）
- 跨平台综合，提取对中国学生有用的信息
